...
// file_put_contents("logs/test5.txt", $ttndtajs);
...
// function voltage to percentage
function voltage_to_percentage($voltage) {
    ...
    if ($voltage >= 3.65) {
        ...
        for ($i = 0; $i < count($voltage_to_soc) - 1; $i++) {
            list($v1, $p1) = $voltage_to_soc[$i];
            list($v2, $p2) = $voltage_to_soc[$i + 1];
            if ($v2 <= $voltage && $voltage <= $v1) {
...
// new payload handler 2024-07-17
// payloads now use measurementGroups, which holds measurements, which holds the values, the sensors and channels are identified by deviceIndex and channelIndex
...
// if there is a measurementGroups in the decoded_payload, use this data to fill the database
if($data["uplink_message"]["decoded_payload"]["measurementGroups"]){
	//Data from the things network (new payload format)
    $eui = $data["end_device_ids"]["dev_eui"]; // same
	$payload = $data["uplink_message"]["decoded_payload"]; // same
    $version = isset_or_null($payload, "payloadEncodingVersion"); // changed
    $ID_offsets = device_ID($eui, $version, 0); // same?
	$ID = $ID_offsets[0]; // same?
	$offsets = json_decode($ID_offsets[1], true); // same?
	$name = $ID_offsets[2]; // same?

	$json = array(NULL, NULL, NULL, NULL, NULL, NULL, NULL); // same
	$data_set = false; // same


// The decoded_payload, holds measurementGroups, which holds measurements, which holds the values
	// the value for battery is the one with deviceIndex 1 and channelIndex 1
	if(isset($payload["measurementGroups"])){
		$measurements = $payload["measurementGroups"][0]["measurements"];
		foreach($measurements as $measurement){
			if($measurement["deviceIndex"] == 1 && $measurement["channelIndex"] == 1){
				// value is between 0 and 1, convert to percentage
				$json[0] = round($measurement["value"] * 100, 0) + $offsets[0];
				// $json[0] = round($measurement["value"], 0) + $offsets[0];
				$data_set = true;
			}
		}
	}
	// the value for temperature is the one with deviceIndex 0 and channelIndex 0
	if(isset($payload["measurementGroups"])){
		$measurements = $payload["measurementGroups"][0]["measurements"];
		foreach($measurements as $measurement){
			if($measurement["deviceIndex"] == 2 && $measurement["channelIndex"] == 0){
				$json[1] = round($measurement["value"], 2) + $offsets[1];
				$data_set = true;
			}
		}
	}
	// the value for humidity is the one with deviceIndex 2 and channelIndex 1
	if(isset($payload["measurementGroups"])){
		$measurements = $payload["measurementGroups"][0]["measurements"];
		foreach($measurements as $measurement){
			if($measurement["deviceIndex"] == 2 && $measurement["channelIndex"] == 1){
				$json[2] = round($measurement["value"], 2) + $offsets[2];
				$data_set = true;
			}
		}
	}
	// the value for illumination is the one with deviceIndex 3 and channelIndex 0
	if(isset($payload["measurementGroups"])){
		$measurements = $payload["measurementGroups"][0]["measurements"];
		foreach($measurements as $measurement){
			if($measurement["deviceIndex"] == 3 && $measurement["channelIndex"] == 0){
				$json[3] = round($measurement["value"], 0) + $offsets[3];
				$data_set = true;
			}
		}
	}
	// pressure, voc and dust are not yet implemented in the new payload format, use NULL for now.
	$json[4] = NULL; 
	$json[5] = NULL;
	$json[6] = NULL;
...
// END new payload, if not working, delete 
// // also delete the elseif below and uncomment the if(isset...-line)




//Receive downlink data from the things network or direct data from gateways. Difference is in the hardware_serial id.
}elseif(isset($data["end_device_ids"])){
// if(isset($data["end_device_ids"])){
	...
	$data_set = false;
// battery is in voltage, convert to percentage
	if(isset($payload["battery"])) {$battery_percentage = voltage_to_percentage($payload["battery"]);
		$json[0] = round($battery_percentage, 0) + $offsets[0];
		$data_set = true;
	}
// end battery is in voltage, convert to percentage. delete if not needed and uncomment the following code
